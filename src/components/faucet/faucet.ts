...
const fixAssets = (candidateAssets: [Asset, Asset]): [Asset, Asset] => {
  ...
  if (
    assets[0].type !== AssetType.NATIVE &&
    assets[1].type !== AssetType.NATIVE
  ) {
...
export const faucetLp = async (sender: Sender, vaultAddress: Address) => {
  ...
  const rawDeDustFactory = DeDustFactory.createFromAddress(
    addresses.dedustFactory
  );
  ...
  const jettonVault = tonClient.open(
    await dedustFactory.getJettonVault(jettonAddress)
  );
  ...
  const jettonRoot = tonClient.open(
    JettonRoot.createFromAddress(jettonAddress)
  );
  const investorJettonWallet = tonClient.open(
